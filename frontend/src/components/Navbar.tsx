    ...
    <header className="sticky top-0 z-40 bg-white/85 backdrop-blur-md border-b border-zinc-200/70 transition-all">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2.5 min-w-0">
          <div className="w-7 h-7 rounded-lg bg-orange-500 flex items-center justify-center text-white shrink-0 shadow-sm">
          ...
          >
            <span className="font-semibold text-sm text-zinc-900 truncate group-hover:text-orange-600 transition">
              {vehicle.year} {vehicle.make} {vehicle.model}
            </span>
            {vehicle.trim && (
              <span className="text-xs text-zinc-400 hidden sm:inline truncate font-normal">
                {vehicle.trim}
              </span>
            )}
          </button>
        ...
        {/* Right Actions: Quiet Summary Badge & Progressive Menu */}
        <div className="flex items-center gap-1 sm:gap-2">
          {/* Subtle Report Button */}
          <button
            onClick={() => setReportModalOpen(true)}
            className="h-8 px-3 rounded-full bg-zinc-100 hover:bg-zinc-200/80 text-zinc-700 text-xs font-medium flex items-center gap-1.5 transition active:scale-95"
          >
              ...
              <div className="absolute right-0 mt-2 w-64 bg-white border border-zinc-200/80 rounded-2xl shadow-xl p-1.5 z-50 animate-in fade-in zoom-in-95 duration-150">
                ...
                <div className="pt-1 border-t border-zinc-100">
                  ...
                  <button
                    onClick={() => {
                      setMenuOpen(false);
                      loadDemoScenario("high_negotiation");
                    }}
                    className="w-full text-left px-3 py-1.5 rounded-lg hover:bg-zinc-50 text-xs text-zinc-700 flex items-center justify-between transition"
                  >
                    <span>Wear & Tear</span>
                    <span className="text-[10px] text-amber-600 font-medium">Negotiate</span>
                  </button>
                </div>
